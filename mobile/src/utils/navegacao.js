...
export const irParaAnterior = async (identificador, navigate) => {
  ...
  for (let i = index - 1; i >= 0; i--) {
    res = await fetch(
      `https://hinario-api.onrender.com/api/hino/${ordemTipos[i]}/ultimo`,
    );
    data = await res.json();
...
export const useSwipe = (onSwipeLeft, onSwipeRight, minDistance = 50) => {
  ...
  const handleTouchMove = (e) => {
    ...
    const diffX = e.nativeEvent.touches[0].pageX - touchStartX.current;
    const diffY = e.nativeEvent.touches[0].pageY - touchStartY.current;
  ...
  const handleTouchEnd = (e) => {
    ...
    const diffX = touchStartX.current - e.nativeEvent.changedTouches[0].pageX;
    const diffY = touchStartY.current - e.nativeEvent.changedTouches[0].pageY;
